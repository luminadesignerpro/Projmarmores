...
async function gerarPdfOrcamento(dados, fetchLogo) {
  ...
  page.drawText('Mármores, Granitos e Importados', { x: textX, y: headerTop - 23, size: 8.5, font: fontRegular, color: GRAY });
  ...
  // ---------------- TÍTULO ----------------
  const titulo = 'ORÇAMENTO / ORDEM DE SERVIÇO';
  ...
  tituloClausula('PRODUTOS E SERVIÇOS');
  const itensList = Array.isArray(itens) && itens.length ? itens : [];
  if (!itensList.length) {
    paragraph('(Nenhum item especificado)', pSize, fontRegular, GRAY, MARGIN, CONTENT_W, pLineH);
  } else {
    itensList.forEach((it, i) => {
      ...
      paragraph(linha, pSize, fontRegular, DARK, MARGIN, CONTENT_W, pLineH);
    });
  }
  y -= 14;

  tituloClausula('VALORES E CONDIÇÕES');
  ...
  if (prazo) paragraph(`Prazo de Entrega: ${prazo}`, pSize, fontRegular, DARK, MARGIN, CONTENT_W, pLineH);
  if (garantia) paragraph(`Garantia: ${garantia}`, pSize, fontRegular, DARK, MARGIN, CONTENT_W, pLineH);
  if (obs) paragraph(`Observações: ${obs}`, pSize, fontRegular, DARK, MARGIN, CONTENT_W, pLineH);
  y -= 4;
  paragraph('Chave Pix para pagamento: 05714829379', pSize, fontRegular, DARK, MARGIN, CONTENT_W, pLineH);
  y -= 14;
  ...
  const sigX = (PAGE_W - colW) / 2;
  page.drawLine({ start: { x: sigX, y }, end: { x: sigX + colW, y }, thickness: 0.8, color: DARK });
  y -= 16;
  centerText((nome || 'CLIENTE').toUpperCase(), sigX, colW, 10, fontBold, DARK);
  y -= 14;
  centerText('DE ACORDO', sigX, colW, 8.5, fontRegular, GRAY);
